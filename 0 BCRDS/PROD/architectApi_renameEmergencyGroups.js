...
const getIds = (arr) => {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].division.id === division.PROD) dataArray.push(arr[i]);
...
client
  ...
  .then(() => {
    return architectApi.getArchitectEmergencygroups(opts);
  })
  .then((data) => {
  ...
  .then((ivrData) => {
    let promises = ivrData.map((body) => {
      ...
      return architectApi.putArchitectEmergencygroup(body.id, newBody);
    });
